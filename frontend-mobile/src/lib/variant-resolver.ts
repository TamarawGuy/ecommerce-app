...
//
// NOTE: this is a verbatim copy of backend/src/lib/variant-resolver.ts (no
// monorepo tooling yet). The backend copy is the canonical, unit-tested source —
// keep the two in sync. It turns a product's variants plus an in-progress
// `{size, color}` selection into everything the detail screen needs: the option
// dimensions to render, which options are still attainable (in stock given the
// other choice), the fully-resolved variant once a complete selection is made,
// and whether the product is in stock at all. It also collapses a single-variant
// product so no picker is shown.
