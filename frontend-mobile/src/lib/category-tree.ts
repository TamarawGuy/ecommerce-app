...
//
// NOTE: this is a verbatim copy of backend/src/lib/category-tree.ts (no monorepo
// tooling yet). The backend copy is the canonical, unit-tested source — keep the
// two in sync. The tree is small, so we fetch the whole flat list once and
// assemble + traverse it in memory rather than using recursive SQL or ltree.
